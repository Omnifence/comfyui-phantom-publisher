...
DRIVER = re.compile(r"^lib(?:cuda\.so|nvidia-|nvcuvid\.|nvidia-encode\.)")
...
def capture(
    ...
    for root, entries in editable_sources.values():
        ...
        if (
            len(root.parts) < 3
            or root == Path.home()
            or re.match(r"^/(?:etc|bin|sbin|lib|usr|opt/venv)(?:/|$)", str(root))
        ):
            raise RuntimeError(
        ...
        for source in entries:
            ...
            files["source/" + source.absolute().as_posix().lstrip("/")] = source
    ...
    for dist in importlib.metadata.distributions():
        ...
        for entry in dist.files:
            ...
            site = next((p for p in sites if absolute.is_relative_to(p)), None)
            if site:
                destination = "site/" + absolute.relative_to(site).as_posix()
    ...
    for root in package_roots:
        for source in root.rglob("*.so*"):
            check_cancelled()
            if source.is_file() and elf(source):
                native_objects.append(source)
    ...
    for name in ("ffmpeg", "ffprobe"):
        ...
        if executable:
            ...
            files["executable/" + source.as_posix().lstrip("/")] = source
            native_objects.append(source)
    ...
    for line in Path("/proc/self/maps").read_text().splitlines():
        ...
        if len(parts) == 6 and parts[5].startswith("/"):
            path = Path(parts[5])
            if ".so" in path.name and path.is_file() and not DRIVER.match(path.name):
                loaded.append(path)
    ...
    for path in libraries.values():
        ...
        files["native/" + path.as_posix().lstrip("/")] = path
